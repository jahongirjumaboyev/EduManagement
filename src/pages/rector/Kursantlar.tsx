...
interface ModalFormState {
  ...
  harbiyUnvoni: string; guvohnomaRaqami: string;
  kursi: string; guruhi: string; lavozimi: string;
}
...
const STUDENTS: Student[] = [
  { id: 1,  fish: 'Aliyev Alisher Baxtiyorovich',          jinsi: 'Erkak', kurs: 1, guruh: 'BI-123' },
  { id: 2,  fish: 'Karimova Malika Saidovna',              jinsi: 'Ayol',  kurs: 1, guruh: 'BI-123' },
  { id: 3,  fish: 'Toshmatov Jasur Umarovich',             jinsi: 'Erkak', kurs: 2, guruh: 'BI-223' },
  { id: 4,  fish: 'Yusupova Nodira Hamidovna',             jinsi: 'Ayol',  kurs: 2, guruh: 'BI-223' },
  { id: 5,  fish: 'Raxmatullayev Sardor Ilhomovich',       jinsi: 'Erkak', kurs: 3, guruh: 'BI-323' },
  { id: 6,  fish: 'Xasanova Zulfiya Davronovna',           jinsi: 'Ayol',  kurs: 3, guruh: 'BI-323' },
  { id: 7,  fish: 'Nazarov Bobur Abdullayevich',           jinsi: 'Erkak', kurs: 4, guruh: 'BI-423' },
  { id: 8,  fish: 'Mirzayeva Dildora Utkurovna',           jinsi: 'Ayol',  kurs: 4, guruh: 'BI-423' },
  { id: 9,  fish: "Holmatov Sherzod Nematovich",           jinsi: 'Erkak', kurs: 1, guruh: 'YUP-123' },
  { id: 10, fish: 'Qodirov Dilshod Xurshidovich',          jinsi: 'Erkak', kurs: 2, guruh: 'YUP-223' },
  { id: 11, fish: 'Abdullayeva Sabohat Farhodovna',        jinsi: 'Ayol',  kurs: 1, guruh: 'YUP-123' },
  { id: 12, fish: 'Ergashev Murod Baxromovich',            jinsi: 'Erkak', kurs: 2, guruh: 'BI-223' },
  { id: 13, fish: 'Hamidova Feruza Tohirovna',             jinsi: 'Ayol',  kurs: 3, guruh: 'BI-323' },
  { id: 14, fish: 'Umarov Firdavs Ismoilovich',            jinsi: 'Erkak', kurs: 4, guruh: 'BI-423' },
  { id: 15, fish: 'Rajabova Mohira Akbarovna',             jinsi: 'Ayol',  kurs: 1, guruh: 'BI-123' },
  { id: 16, fish: "Norqo'ziyev Asilbek Ravshanbekovich",  jinsi: 'Erkak', kurs: 2, guruh: 'BI-223' },
  { id: 17, fish: 'Sobirov Ulugbek Mahmudovich',           jinsi: 'Erkak', kurs: 3, guruh: 'BI-323' },
  { id: 18, fish: 'Tursunova Kamola Alijonovna',           jinsi: 'Ayol',  kurs: 4, guruh: 'BI-423' },
  { id: 19, fish: "Isoqov Laziz Bahodir o'g'li",          jinsi: 'Erkak', kurs: 1, guruh: 'YUP-123' },
  { id: 20, fish: 'Mansurova Hulkar Iskandarovna',         jinsi: 'Ayol',  kurs: 2, guruh: 'YUP-223' },
  { id: 21, fish: 'Xoliqov Sanjar Tursunovich',            jinsi: 'Erkak', kurs: 3, guruh: 'BI-323' },
  { id: 22, fish: 'Baxtiyorova Shahnoza Komilovna',        jinsi: 'Ayol',  kurs: 4, guruh: 'BI-423' },
  { id: 23, fish: 'Yunusov Sirojiddin Abdulhamidovich',    jinsi: 'Erkak', kurs: 1, guruh: 'BI-123' },
  { id: 24, fish: 'Razzaqova Nargiza Rustamovna',          jinsi: 'Ayol',  kurs: 2, guruh: 'BI-223' },
  { id: 25, fish: "Ortiqov Javlon Xurshidovich",           jinsi: 'Erkak', kurs: 3, guruh: 'YUP-223' },
];
...
const EMPTY_MODAL: ModalFormState = {
  ...
  phones: [''], emails: [''],
  harbiyUnvoni: '', guvohnomaRaqami: '', kursi: '', guruhi: '', lavozimi: '',
};
...
const MENU_ITEMS: SidebarItem[] = [
  ...
  { icon: <FileTextOutlined />, label: UZ.darsJadvali.hisobotlar,     path: '', hasArrow: true },
  {
    ...
    path: '',
    subItems: [
      { label: UZ.kursantlar.menuLabel,         path: '/rahbariyat/kursant-tinglovchilar/kursantlar',    active: true },
...
export default function Kursantlar() {
  ...
  const filtered = STUDENTS.filter(s => {
    if (filterKurs && s.kurs !== Number(filterKurs)) return false;
  ...
  const handleModalSubmit = () => {
    ...
    if (!modalForm.harbiyUnvoni)                      e.harbiyUnvoni   = R.majburiy;
    if (!modalForm.guvohnomaRaqami.trim())            e.guvohnomaRaqami = R.majburiy;
    if (!modalForm.kursi)                             e.kursi          = R.majburiy;
    if (!modalForm.guruhi)                            e.guruhi         = R.majburiy;
    if (!modalForm.lavozimi)                          e.lavozimi       = R.majburiy;
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FA]">
      ...
      <div className="lg:ml-60 flex flex-col min-h-screen">
        ...
        <main className="flex-1 p-3 sm:p-4 lg:p-6">
          ...
          <div className="flex items-center justify-between mb-4 sm:mb-5">
            <h1 className="text-base sm:text-lg font-bold text-gray-800">{UZ.kursantlar.pageTitle}</h1>
            <button
            ...
            >
              <UserAddOutlined />
              <span className="hidden sm:inline">{UZ.kursantlar.qoshish}</span>
            </button>
          </div>
          ...
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col gap-1">
                <label className="text-xs text-gray-500">{UZ.kursantlar.oquvYili}</label>
                <select
                  value={filterYear}
              ...
              <div className="flex flex-col gap-1">
                <label className="text-xs text-gray-500">{UZ.kursantlar.kurs}</label>
                <select
                  value={filterKurs}
                  onChange={e => { setFilterKurs(e.target.value); setPage(1); }}
                  className="w-44 border border-gray-300 rounded-lg px-3 py-2 text-sm text-gray-700 bg-white outline-none"
                >
                  <option value="">— {UZ.kursantlar.kurs} —</option>
                  {UZ.kursantlar.kursOptions.map((k, i) => (
                    <option key={k} value={String(i + 1)}>{k}</option>
                  ))}
                </select>
              </div>

              <div className="flex flex-col gap-1">
                <label className="text-xs text-gray-500">{UZ.kursantlar.oquvGuruhi}</label>
                <select
                  value={filterGuruh}
                  onChange={e => { setFilterGuruh(e.target.value); setPage(1); }}
                  className="w-44 border border-gray-300 rounded-lg px-3 py-2 text-sm text-gray-700 bg-white outline-none"
                >
                  <option value="">— {UZ.kursantlar.oquvGuruhi} —</option>
                  {UZ.kursantlar.guruhOptions.map(g => (
              ...
              <div className="flex flex-col gap-1">
                <label className="text-xs text-gray-500">{UZ.kursantlar.fish}</label>
                <input
                  type="text"
                  value={filterFish}
                  onChange={e => { setFilterFish(e.target.value); setPage(1); }}
                  placeholder={UZ.kursantlar.fish}
                  className="w-44 border border-gray-300 rounded-lg px-3 py-2 text-sm text-gray-700 bg-white outline-none"
                />
              </div>

              <div className="flex flex-col gap-1">
                <label className="text-xs text-gray-500">{UZ.kursantlar.jinsi}</label>
                <select
                  value={filterJinsi}
                ...
                >
                  <option value="">—</option>
                  {UZ.kursantlar.jinsOptions.map(j => (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-[#F8F9FA] border-b border-gray-200">
                    <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 w-12">{UZ.kursantlar.tr}</th>
                    <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500">{UZ.kursantlar.fish}</th>
                    <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500">{UZ.kursantlar.jinsi}</th>
                    <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500">{UZ.kursantlar.kurs}</th>
                    <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500">{UZ.kursantlar.oquvGuruhi}</th>
                    <th className="w-16" />
                  </tr>
                </thead>
                <tbody>
                  {pageStudents.map((s, i) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-gray-600">{UZ.kursantlar.kursOptions[s.kurs - 1]}</td>
      ...
      {/* ── Modal: Kursant qo'shish / ko'rish ── */}
      <Modal
        open={isModalOpen}
        onCancel={closeModal}
        footer={null}
        ...
        <div className="border-t-4 border-[#3380FF] px-6 py-5">
          ...
          <div className="flex justify-between items-center mb-6">
            <span className="text-lg font-bold text-gray-900">
              {isSaved ? UZ.kursantlar.viewTitle : UZ.kursantlar.kursantQoshishTitle}
            </span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div>
              <FieldLabel text={UZ.kursantlar.harbiyUnvoniLabel} required />
              {isSaved ? <div className={viewCls}>{savedData?.harbiyUnvoni}</div> : (
                <>
                  <Select className="w-full" size="middle" placeholder={UZ.royxatgaOlish.tanlang}
                    options={toOpts(UZ.kursantlar.harbiyUnvoniOptions)}
                    value={modalForm.harbiyUnvoni || undefined}
                    onChange={v => setM('harbiyUnvoni', v)} />
                  {modalErrors.harbiyUnvoni && <p className="text-xs text-red-500 mt-0.5">{modalErrors.harbiyUnvoni}</p>}
                </>
              )}
            </div>

            <div>
              <FieldLabel text={UZ.kursantlar.guvohnomaRaqamiLabel} required />
              {isSaved ? <div className={viewCls}>{savedData?.guvohnomaRaqami}</div> : (
                <>
                  <input type="text" value={modalForm.guvohnomaRaqami}
                    ...
                    placeholder={UZ.royxatgaOlish.guvohnomaPlaceholder} className={inputCls} />
            ...
            <div>
              <FieldLabel text={UZ.kursantlar.kursiLabel} required />
              {isSaved ? <div className={viewCls}>{savedData?.kursi}</div> : (
                <>
                  <Select className="w-full" size="middle" placeholder={UZ.royxatgaOlish.tanlang}
                    options={toOpts(UZ.kursantlar.kursOptions)}
                    value={modalForm.kursi || undefined}
                    onChange={v => setM('kursi', v)} />
                  {modalErrors.kursi && <p className="text-xs text-red-500 mt-0.5">{modalErrors.kursi}</p>}
                </>
              )}
            </div>

            <div>
              <FieldLabel text={UZ.kursantlar.guruhiLabel} required />
              {isSaved ? <div className={viewCls}>{savedData?.guruhi}</div> : (
                <>
                  <input type="text" value={modalForm.guruhi}
                    ...
                    placeholder={UZ.kursantlar.guruhiPlaceholder} className={inputCls} />
            ...
            <div>
              <FieldLabel text={UZ.kursantlar.lavozimLabel} required />
              {isSaved ? <div className={viewCls}>{savedData?.lavozimi}</div> : (
                <>
                  <Select className="w-full" size="middle" placeholder={UZ.royxatgaOlish.tanlang}
                    options={toOpts(UZ.kursantlar.lavozimOptions)}
                    value={modalForm.lavozimi || undefined}
                    onChange={v => setM('lavozimi', v)} />
                  {modalErrors.lavozimi && <p className="text-xs text-red-500 mt-0.5">{modalErrors.lavozimi}</p>}
                </>
              )}
            </div>
